...
const makeSafe = (string, headingIds, prefix) => {
  const key = S(prefix).slugify().s +'-'+S(string).slugify().s // slugify
  if (!headingIds[key]) {
    headingIds[key] = 0
  }
  headingIds[key]++
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      if (heading.type === "inline") {
        ...
        if (heading.children && heading.children[0].type === "link_open") {
          ...
          heading._tocAnchor = makeSafe(content,
            headingIds,
            state.env.anchorPrefix || md.options.anchorPrefix)
        }
        else {
